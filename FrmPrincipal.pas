...
type
  ...
  TAno = class
    ...
    procedure CalcularCorpusChristi(ano: TDateTime);
  end;
...
begin
  ano := ano - 47;
  ShowMessage('O carnaval cairá no dia: ' + DateToStr(ano));
end;

procedure TAno.CalcularCorpusChristi(ano: TDateTime);
begin
  ano := ano + 60;
  ShowMessage('O feriado de Corpus Christi cairá no dia: ' + DateToStr(ano));
end;

procedure TAno.CalcularSextaFeiraSanta(ano: TDateTime);
begin
  ano := ano - 2;
  ShowMessage('A Sexta-Feira da paixão cairá no dia: ' + DateToStr(ano));
end;
